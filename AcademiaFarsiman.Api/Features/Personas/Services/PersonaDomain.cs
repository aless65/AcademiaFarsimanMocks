...
namespace AcademiaFarsiman.Api.Features.Personas.Services
{
    ...
    {
        public Respuesta<List<Persona>> ValidarRespuestaObtener(List<Persona> personas)
        {
            if (personas is null || !personas.Any())
            {
                return Respuesta<List<Persona>>.Fault(MensajesPersonas.MSP001);
            }

            return Respuesta<List<Persona>>.Success(personas);
        }
