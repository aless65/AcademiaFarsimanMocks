...
namespace AcademiaFarsiman.Api.Features.Personas.Services
{
    public class PersonasService : IPersonasService
    {
        ...
        public PersonasService(IUnitOfWork unitOfWork,
                               PersonaDomain personasDomain,
                               IMapper mapper)
        {
            _unitOfWork = unitOfWork;
            _dominio = personasDomain;
            _mapper = mapper;
        }
        ...
        public async Task<Respuesta<List<PersonaDto>>> Obtener()
        {
            try
            {
                ...
                var respuestaValidacion = _dominio.ValidarRespuestaObtener(personas);

                if (!respuestaValidacion.Ok)
                {
                    return Respuesta<List<PersonaDto>>.Fault(respuestaValidacion.Mensaje);
                }
